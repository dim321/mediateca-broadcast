...
### Shows per hour
Integer N on a media plan and on an advertising order. Planned play time for a clock hour is the sum of N clip durations from the rotation's ordered catalog, cycling position modulo catalog size (not `N × full cycle sum`). Multiple commercial plans in the same hour sum without subtracting real on-screen overlap (soft MVP). When the quota period unit is day, checks still slice by hour within the day. An advertising order stores one N for the whole document; activation copies it onto each occupied claim.
