...
## Phase 8 (polish) — синхронизация с кодом

- **Avo**: ресурсы `MediaAsset`, `Playlist`, `BroadcastPoint`, `ScheduleRule` в `app/avo/resources/`; базовый класс задаёт изоляцию по `organization_id` (OSS Avo без `avo-pro` не применяет Pundit к выборкам автоматически). Контроллер `app/controllers/avo/application_controller.rb` выставляет `Current.user` / `Current.organization` из сессии кабинета.
- **Pundit**: тесты границ тенанта в `spec/policies/`.
- **Индексы**: см. миграцию `db/migrate/*_add_indexes_and_constraints_refinements.rb` (композитные индексы под медиа, расписания, точки).
- **Device API**: идентификаторы в JSON — bigint из БД; контракт использует плейсхолдер `uuid` как «стабильный внешний id» — см. примечание в `contracts/device-api-v1.md`.
