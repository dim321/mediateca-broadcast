...
## Phase 8 — валидация (2026-04-08)

- Полный прогон `bundle exec rspec`: **110 примеров, 0 сбоев**; миграции индексов применены (`db:migrate`).
- Админка **Avo** (`/avo`): скоуп по `Current.organization` через контекст; без сессии кабинета записи с `organization_id` недоступны (пустые списки).
- Дымовые проверки числа SQL: `spec/requests/performance/filtering_spec.rb` (фильтр точек по тегам, список расписаний).
