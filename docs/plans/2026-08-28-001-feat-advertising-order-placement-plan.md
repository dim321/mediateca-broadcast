...
- [x] `ReplaceClip` + UI (выбор ролика из готовых, предупреждение о хронометраже, подтверждение), инкремент `document_version`.
- [x] Тесты: domain (swap, отказ на not-ready ролике, версия), request, system «заменить → в пакете станции новый ролик».
- [x] DoD Unit: спеки зелёные; id слотов неизменны в тесте AE7.

### U5. Печатная форма

- [x] Print layout + `Advertising::PrintSheetComponent` + print CSS (`@page`, блоки месяцев по ТЗ, прочерки, итоги, подписи, версия).
- [x] Тесты: component-spec (блоки месяцев: 25.12–05.01 → 2 блока; 20 дней через месяц → 1 блок; прочерки), request print 200 для A1–A4.
- [x] DoD Unit: визуальная сверка с PDF «Триумф» (структурно), спеки зелёные.
