...
RSpec.describe Portraits::CopyTemplate do
  ...
  it "returns nil and does not create a portrait when there is no default template" do
    ...
    expect(described_class.call(screen: screen)).to be_nil
    expect(screen.reload.broadcast_portrait).to be_nil
  end
  ...
  it "copies the default template and its blocks onto the screen" do
    ...
    portrait = described_class.call(screen: screen)
  ...
  it "does not cascade later template edits onto the copy" do
    ...
    described_class.call(screen: screen)

    template.update!(name: "Night grid")

    expect(screen.reload.broadcast_portrait.name).to eq("Grid")
  end

  it "returns the existing screen portrait without duplicating" do
    create(:broadcast_portrait, :default)
    existing = create(:broadcast_portrait, :for_screen, screen: screen)

    expect(described_class.call(screen: screen)).to eq(existing)
  ...
  it "replaces an existing screen portrait and enqueues regen" do
    ...
    expect {
      described_class.call(screen: screen, template: replacement, replace: true)
