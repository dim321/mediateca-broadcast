...
RSpec.describe "Schedule rules UI", type: :system do
  ...
  let(:rotation) { create(:rotation, organization: org, name: "Morning mix") }
  ...
  it "creates a schedule and blocks overlapping ranges" do
    ...
    select rotation.name, from: "schedule_rule_rotation_id"
    ...
    expect(page).to have_content(rotation.name)
